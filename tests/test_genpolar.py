...
class TestTask12:
    ...
    def test_rtrings_correct(self, genpolar):
        positions = np.array(list(genpolar.rtrings(rmax=5., nrings=5, multi=6)), dtype=np.float64)
        expected_positions = np.array([(0.0, 0.0),
                                       ...
                                       (5.0, 6.073745796940266)], dtype=np.float64)

        sorted_positions = positions[np.lexsort((positions[:, 1], positions[:, 0]))]
        sorted_expected_positions = expected_positions[np.lexsort((expected_positions[:, 1], expected_positions[:, 0]))]
